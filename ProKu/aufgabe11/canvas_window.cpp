...
namespace Proku
{
  ...
  {
    XColor xcolour;
    xcolour.red   = static_cast<unsigned short>(red   <= 0.0 ? 0 : (red   >= 1.0 ? 65535 : int(red  *65535.0)));
    xcolour.green = static_cast<unsigned short>(green <= 0.0 ? 0 : (green >= 1.0 ? 65535 : int(green*65535.0)));
    xcolour.blue  = static_cast<unsigned short>(blue  <= 0.0 ? 0 : (blue  >= 1.0 ? 65535 : int(blue *65535.0)));
  ...
  {
    XDrawString(display, pixmap, gcontext, x, y, text.data(), int(text.size()));
  }
