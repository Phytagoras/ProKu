...
float* read_jpeg(unsigned int& width, unsigned int& height,
    ...
    if (file == nullptr) {
        std::cerr << "FEHLER: konnte Datei '" << filename << "' nicht oeffnen!"
                  << std::endl;
        return nullptr;
    }
    ...
    for (unsigned int i(0); i < height; ++i) rows[i] = &buffer[i * width];
...
bool write_jpeg(const unsigned int width, const unsigned int height,
    ...
    if ((width < 8) || (width > 8192)) {
        std::cerr << "FEHLER: ungueltige Bild-Breite; unterstuetzt: 8 - 8192"
                  << std::endl;
        return false;
    }
    if ((height < 8) || (height > 8192)) {
        std::cerr << "FEHLER: ungueltige Bild-Hoehe; unterstuetzt: 8 - 8192"
                  << std::endl;
        return false;
    }
    if (pixels == nullptr) {
    ...
    if (file == nullptr) {
        std::cerr << "FEHLER: konnte Datei '" << filename
                  << "' nicht erstellen!" << std::endl;
        return false;
    }
